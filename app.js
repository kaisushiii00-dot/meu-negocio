(() => {
  ...
  function renderEstoque(d) {
    const filter = state.estoqueFilter || "todos";
    const categorias = [...new Set(d.products.map((p) => p.category || "Geral"))];
    const list = d.products.filter((p) => filter === "todos" || (p.category || "Geral") === filter);
    return `
      <div class="toolbar">
        <div class="pill-group" style="flex-wrap:wrap;row-gap:6px">
          <button class="pill ${filter === "todos" ? "active" : ""}" data-filter-stock="todos">Todos</button>
          ${categorias.map((c) => `<button class="pill ${filter === c ? "active" : ""}" data-filter-stock="${esc(c)}">${esc(c)}</button>`).join("")}
        </div>
        <button class="btn-primary" data-open-modal="novo-produto">${ICONS.plus}Novo produto</button>
      </div>
      <div class="card">
        ${list.length === 0 ? `<p class="empty">Nenhum produto nesta categoria.</p>` : list.map((p) => {
          const low = p.stock <= p.minStock;
          return `
          <div class="row">
            <div style="display:flex;gap:10px;align-items:center;min-width:0">
              <div class="row-thumb">${p.photo ? `<img src="${p.photo}" alt="" />` : `<span>${esc((p.name || "?").charAt(0).toUpperCase())}</span>`}</div>
              <div style="min-width:0">
                <div class="row-title" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(p.name)}</div>
    ...
    return `
      ...
      <div class="modal-body">
        <label class="field"><span>Foto (opcional)</span>
          <input type="file" accept="image/*" id="prod-photo-input" />
          <div id="prod-photo-preview" style="margin-top:8px"></div>
        </label>
        <label class="field"><span>Nome do produto</span><input id="prod-name" /></label>
        <label class="field"><span>Categoria</span><input id="prod-category" list="prod-category-list" /><datalist id="prod-category-list">${[...new Set((state.data.products || []).map((p) => p.category || "Geral"))].map((c) => `<option value="${esc(c)}">`).join("")}</datalist></label>
  ...
  function bindEvents(d) {
    ...
    document.querySelectorAll("[data-open-modal]").forEach((btn) => btn.addEventListener("click", () => {
      ...
      if (btn.dataset.openModal === "novo-produto") state.novoProdutoPhoto = null;
    ...
    // Novo produto
    document.getElementById("prod-photo-input")?.addEventListener("change", (e) => {
      ...
      reader.onload = (ev) => {
        ...
        img.onload = () => {
          ...
          state.novoProdutoPhoto = dataUrl;
          const preview = document.getElementById("prod-photo-preview");
          if (preview) preview.innerHTML = `<img src="${dataUrl}" style="width:64px;height:64px;object-fit:cover;border-radius:8px;border:1px solid var(--rule)" />`;
        };
    ...
    document.getElementById("salvar-produto")?.addEventListener("click", () => {
      ...
      const p = {
        ...
        photo: state.novoProdutoPhoto || null,
      };
      state.novoProdutoPhoto = null;
      state.modal = null;
    ...
    document.querySelectorAll("[data-filter-stock]").forEach((btn) => btn.addEventListener("click", () => {
      state.estoqueFilter = btn.dataset.filterStock; render();
    }));
